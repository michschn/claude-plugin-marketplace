...
// All widgets as direct children of screen
```

## When to Consult @lvgl-expert

Invoke the full agent (`@lvgl-expert`) for:
- Architecture decisions for complex UIs
- Multi-screen navigation design
- Performance optimization for large UIs
- When you need extended thinking on widget selection
- Reviewing significant UI code changes

## Knowledge Cache

Check `.skills/lvgl/knowledge/` for cached documentation:
- `SUMMARY.md` - Comprehensive widget summary
- `metadata.json` - Cache freshness info

If cache is stale (>7 days) or missing needed widget info, recommend running `/lvgl-update-docs`.
